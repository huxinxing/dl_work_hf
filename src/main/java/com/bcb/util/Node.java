...
public class Node {
     ...
     * @return root
     */
    public Node createTree(List<Node> Nodes,String roots) {
        if (Nodes == null || Nodes.size() < 0)
            return null;
        Node root = new Node("root",roots);//根节点自定义，但是要和pid对应好
        // 将所有节点添加到多叉树中
        for (Node node : Nodes) {
            if (node.getPid().equals(roots) || node.getPid().equals("root")) {//根节点自定义，但是要和pid对应好
                // 向根添加一个节点
    ...
    public void addChild(Node Node, Node child) {
        for (com.bcb.util.Node item : Node.getNodes()) {
    ...
    public String iteratorTree(Node Node) {
        ...
        buffer.append("\n");
        if (Node != null) {
            for (com.bcb.util.Node index : Node.getNodes()) {
    ...
    /*public static void main(String[] args) {
        ...
        Node tree = new Node();
        Node mt = tree.createTree(nodes,"root");
