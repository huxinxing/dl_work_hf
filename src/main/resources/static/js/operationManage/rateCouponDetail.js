...
var innerFunc = {
    ...
    tableShow : function(){
        layui.use('table', function(){
            ...
            table.render({
                elem: '#transFlowTable'
                ,method:'post'
                ,where:{icStatus: $('#paymentType').val(),userId:$('#walletAddr').val(),icId:sessionStorage.getItem('geId')}
                // ,width:1200

                ,url: '/operarions/icUserList'
                ,headers:{
                    "Authorization":localStorage.getItem("loginToken")
                }
                ,cols: [[
    ...
    saveProject : function () {
        ...
        paramter.icId = sessionStorage.getItem("geId");
        paramter.icStatus = $('#status_rateCouDetail').val();
        paramter.icRate = geAmount;
        paramter.icLength = experienceLength;
        paramter.icProject = ids;
        innderAjax('post',"/operarions/icUpdate",JSON.stringify(paramter),function (resp) {
            if(resp.code === '200'){
...
var initData = function(){
    innderAjax('post',"/operarions/icdetail",{icId:sessionStorage.getItem('geId')},function(r){
        if(r.code == '200'){
            $('#amount_1').val(r.data.icRate);
            $('#amount_2').val(r.data.icLength);
            // var liItemLength = $('.select-items .li-item').length;
            $('.createTime').text(r.data.icCreateTime);
            $('.status').val(r.data.icStatus);
            var validityDay = r.data.icValidityDay;
            if(!(validityDay === null)){
                $('#fixed_day')[0].checked = true;
                $('#amount_4').val(r.data.icValidityDay)
            }else{
                $('.btn-radio').eq(1).trigger('click');
                // $('#fixed_date')[0].checked = true;
                $('#amount_4').val(r.data.icValidityDay);
                $('#createTimeStart').val(r.data.icValidityBeginTime);
                $('#createTimeEnd').val(r.data.icValidityEndTime);
            }
            var liArr = [],html = '',liObj = r.data.icProject;
            for(var i in liObj){
